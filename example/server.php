...
function server_start($hostname, $port, $router, $sn) {
	...
	if (substr(PHP_OS, 0, 3) == 'WIN') {
		...
		$cmd = "exec {$php_executable} -t {$doc_root} -n -d extension=molten.so -d molten.enable=1 -d molten.sink_type=4 -d molten.sink_http_uri=http://127.0.0.1:9411/api/v1/spans -d molten.span_id_format=random -d molten.service_name={$sn} -d molten.sampling_rate_base=1  -S " . $address;
		$cmd .= " {$router}";
		$cmd .= " 2>/dev/null";
